from parameters_inductance import *
...
#distance_list = [0.004, 0.003, 0.0028, 0.0025, 0.0022, 0.0021, 0.00205]
...
#param_sweep = 'frequency'
rs = 0.001
# -----------------------------------
# DON'T CHANGE ANYTHING BELOW
# -----------------------------------

if param_sweep == 'distance':
    ...
    input_static_servername = 'Input/2Model Excitation/0Frequency/Hz'
    input_static_value = int(1e4)  #freq_list[0]

    # input variables to be swept
    str_varname = 'Distance'
    unit_scale = 1e-3
    str_var_units = '[mm]'
    input_servername = 'Input/0Geometry Parameters/4Distance Between Conductors in X/[m]'
...
def set_method(c, solve_method):
    ...
    isthin = c.defineNumber(servername_isthin, value=isthin_val, choices=[0, 1, 2, 3])
    #set_variable(c, servername_isthin, isthin)


def set_numwires(c, numwires):
    set_variable(c, servername_columns, numwires)
    print('This model has', c.getNumber(servername_columns), 'wires')
...
def calculate_impedance_matrix(c, getdp, model_pro, model_msh):
    ...
    for j in range(1, numwires + 1):
        ...
        for i in range(1, numwires + 1):
            ...
            if i == j:
                resistance = c.getNumber(servername_resistance + smethod_str + ext_ij)
            elif (i != j) and (solve_method == 'thin'):
                resistance_prox = c.getNumber(servername_resistance_p + smethod_str + '-Prox'+ ext_ij)
                # print(servername_resistance_p + smethod_str + '-Prox'+ ext_ij)

            inductance = c.getNumber(servername_inductance + smethod_str + ext_ij)
...
def variable_sweep_getdp(c, gmsh, getdp, model_geo, model_pro, model_msh, variable_list):
    ...
    for n, var in enumerate(variable_list):
        ...
        active_distx = c.getNumber(servername_distx)

        # create strings to label output file
        if param_sweep is not 'frequency':
            freq_str = '_freq_' + str(int(active_freq))
        else:
            freq_str = ''

        if param_sweep is not 'distance':
            dist_str = '_dist_' + str(active_distx)
        else:
            dist_str = ''

        if param_sweep is not 'sleeve':
        ...
        if resistance_prox_label_str:
            print(str(active_freq) + '\t' + str(active_distx) + '\t' + str(active_rs) +
