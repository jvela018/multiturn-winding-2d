...
Else
    ...
    AirPoints[] += newp; Point(newp) = {-0.01, -0.01, 0, msh_air};
    AirPoints[] += newp; Point(newp) = {0.01, -0.01, 0, msh_air};
    AirPoints[] += newp; Point(newp) = {0.01, 0.01, 0, msh_air};
    AirPoints[] += newp; Point(newp) = {-0.01, 0.01, 0, msh_air};
    ...
    AirLines[] += newl; Circle(newl) = {AirPoints[0], CenterPoints[0] , AirPoints[1]};
    AirLines[] += newl; Circle(newl) = {AirPoints[1], CenterPoints[0] , AirPoints[2]};
    AirLines[] += newl; Circle(newl) = {AirPoints[2], CenterPoints[0] , AirPoints[3]};
    AirLines[] += newl; Circle(newl) = {AirPoints[3], CenterPoints[0] , AirPoints[0]};
...
If(isThin && withRing)
    ...
    For i In {1:#CenterPoints[]}  
        Transfinite Curve {OutlinesLines[{0+sleeve_mesh_index,1+sleeve_mesh_index,2+sleeve_mesh_index,3+sleeve_mesh_index}]} = 100 Using Progression 1;
        sleeve_mesh_index += 4;
    EndFor
EndIf
...
If(!isThin)
    sleeve_mesh_index = 0 ;
    For i In {1:#CenterPoints[]}  
      Transfinite Curve {OutlinesLines[{0+sleeve_mesh_index,1+sleeve_mesh_index,2+sleeve_mesh_index,3+sleeve_mesh_index}]} = 100 Using Progression 1; //35
        sleeve_mesh_index += 4;
    EndFor
EndIf

// Define boundary layers / mesh control for volume conductors
If(!isThin && isBoundaryLayer)   
    Field[1] = BoundaryLayer;
    Field[1].EdgesList =  {Boundary{ Surface{ConductorSurface[]};}};
    Field[1].hfar = rad/8;                      // Element size far from the wall
    Field[1].hwall_n = rad/800;                 // Mesh Size Normal to the The Wall
    Field[1].thickness = rad/5;                  // Maximum thickness of the boundary layer
...
// Air mesh control
If(AirAxi)
    Transfinite Curve {AirLines[{0,1}]} = 30 Using Progression 1;
    Transfinite Curve {AirLines[{2,3}]} = 50 Using Progression 1;
Else
    Transfinite Curve {AirLines[{0,1,2,3}]} = 100 Using Progression 1;
EndIf
